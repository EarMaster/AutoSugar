...
**Mandatory per-locale check — do this for every locale, not just English:** after writing (or translating) each `docs/whatsnew/X.Y.Z-{LOCALE}` file, check that specific file's character count with a UTF-8 locale forced — `LC_ALL=C.UTF-8 wc -m <file>`. Bare `wc -m` silently counts *bytes* when the shell has no UTF-8 locale, which is the default for the non-interactive shells this command runs in on Windows, and `wc -c` always counts bytes. Either way multi-byte locales like `ar`/`hi-IN`/`ja-JP`/`zh-CN` measure two to three times their real length and you will trim text that was never over the limit. Cross-check anything close to the limit with `python -c "import io,sys;print(len(io.open(sys.argv[1],encoding='utf-8').read()))" <file>`. No locale file may exceed 500 characters — Google Play's hard "what's new" limit is per-locale, not per-release. If any file is over, re-trim that locale's text (summarize/combine bullets) and re-check — do not assume it's fine because the English source was short.
